...
pub fn create(app: &tauri::App, settings: &crate::settings::Settings) -> tauri::Result<()> {
    ...
    let size_100 = CheckMenuItem::with_id(app, "size-100", "尺寸 100%（原尺寸）", true, settings.scale_percent == 100, None::<&str>)?;
    let size_150 = CheckMenuItem::with_id(app, "size-150", "尺寸 150%", true, settings.scale_percent == 150, None::<&str>)?;
    let size_200 = CheckMenuItem::with_id(app, "size-200", "尺寸 200%（更清晰）", true, settings.scale_percent == 200, None::<&str>)?;
    let size_300 = CheckMenuItem::with_id(app, "size-300", "尺寸 300%（展示）", true, settings.scale_percent == 300, None::<&str>)?;
    let exit = MenuItem::with_id(app, "exit", "退出", true, None::<&str>)?;
    let separator_1 = PredefinedMenuItem::separator(app)?;
    let separator_2 = PredefinedMenuItem::separator(app)?;
    let menu = Menu::with_items(app, &[&open, &confirm, &sound, &separator_1, &size_100, &size_150, &size_200, &size_300, &separator_2, &exit])?;
...
fn sync_checks(app: &AppHandle, settings: &crate::settings::Settings) {
